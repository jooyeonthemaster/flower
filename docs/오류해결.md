...
마지막 해결된 오류:
- **오류 #21**: 로컬 환경 변수 누락 → ✅ 해결 완료 (2026-01-23)
- **오류 #20**: Vercel FFmpeg 미작동 → ✅ 해결 완료 (2026-01-23)
- **오류 #19**: 413 Payload Too Large → ✅ 해결 완료 (2026-01-23)
- **오류 #18**: Firebase Storage CORS 에러 → ✅ 해결 완료 (2026-01-23)

---

## 최근 해결된 오류 (2026-01-23)
...
- 로컬에서 AI 영상 생성 테스트 시 "Higgsfield API credentials not configured" 에러
- 이전에 잘 되던 기능이 갑자기 작동 안 함

#### 원인

- `.env.local` 파일이 덮어써지면서 HIGGSFIELD 관련 키가 삭제됨
- Vercel에서 HIGGSFIELD_API_KEY, HIGGSFIELD_API_SECRET이 **Production 환경에만** 설정됨
- `vercel env pull`은 Development 환경 변수만 가져옴

#### 해결 방법

1. Production 환경 변수 가져오기:
   ```bash
   npx vercel env pull .env.production --environment=production
   ```

2. HIGGSFIELD 키와 FIREBASE_SERVICE_ACCOUNT_KEY를 `.env.local`에 수동 추가

#### 교훈

- `.env.local` 수정 시 기존 내용 보존 필수 (Write 대신 Edit 사용)
- Vercel 환경 변수 설정 시 Development 환경도 체크할 것

---

### 오류 #20: Vercel FFmpeg 미작동 ✅

**발생일**: 2026-01-23
**완료일**: 2026-01-23
**파일**: `next.config.ts`, `vercel.json`
**상태**: ✅ 해결 완료

#### 문제 상황

- Vercel 배포 후 영상 합성(merge) 시 3개 영상이 아닌 1개만 반환
- 로컬에서는 정상 작동

#### 원인

- Next.js 빌드 시 ffmpeg-static 바이너리가 번들에서 제외됨
- FFmpeg를 찾지 못하면 첫 번째 영상만 반환하는 fallback 로직 실행
...
### 오류 #19: 413 Payload Too Large ✅

**발생일**: 2026-01-23
**완료일**: 2026-01-23
**파일**: `check-video-status/route.ts`, `merge-videos/route.ts`
**상태**: ✅ 해결 완료
...
### 오류 #18: Firebase Storage CORS 에러 ✅
...
- [413 Payload Too Large 해결](./archive/CORS-413-ERROR-FIX.md) - 오류 #18, #19 ✅
- [Vercel FFmpeg 작동 문제](./archive/FFMPEG-MERGE-BUG-ANALYSIS.md) - 오류 #20 ✅
