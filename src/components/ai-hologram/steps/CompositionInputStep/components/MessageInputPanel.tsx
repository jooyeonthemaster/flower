...
export default function MessageInputPanel({
  ...
  return (
    <div className={`flex flex-col h-full gap-4 min-h-0 ${hideButtons ? 'w-full' : ''}`}>
      <div className={`flex-1 flex flex-col bg-gradient-to-br from-slate-900/80 to-black/80 border border-amber-500/20 rounded-[1.5rem] overflow-hidden shadow-[0_0_40px_-10px_rgba(251,191,36,0.05)] backdrop-blur-md min-h-0 ${hideButtons ? 'bg-transparent border-none shadow-none rounded-none' : ''}`}>
        {/* 헤더 (hideButtons일 때는 컨텍스트가 다르므로 숨기거나 심플하게) */}
        {!hideButtons && (
        ...
        {/* hideButtons true일 때 상단 탭만 따로 렌더링 (헤더 없이) */}
        {hideButtons && (
        ...
        {/* 컨텐츠 스크롤 영역 */}
        <div className={`flex-1 overflow-y-auto custom-scrollbar ${hideButtons ? 'p-0' : 'p-6 pt-2'}`}>
          {/* 키워드 추천 영역 */}
          {messageMode !== 'custom' && (
              ...
              <div key={index} className="bg-black/20 rounded-xl p-3 border border-white/5">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <span
                      className={`w-5 h-5 rounded flex items-center justify-center text-xs font-bold ${message.trim() ? 'bg-amber-500 text-black' : 'bg-slate-800 text-gray-500'}`}
                    >
                      {index + 1}
                    </span>
                    <span className="text-xs font-bold text-gray-300">{sceneLabels[index]?.label}</span>
                  </div>
                  <span className="text-[10px] text-amber-500/70 bg-amber-500/10 px-1.5 py-0.5 rounded border border-amber-500/10">10초</span>
                </div>
                <textarea
                  value={message}
                  ...
                  rows={2}
                  className="w-full bg-black/40 rounded-lg border border-white/5 focus:border-amber-500/50 focus:bg-black/60 outline-none text-sm text-white placeholder-gray-600 p-3 resize-none transition-all leading-snug"
                />
              </div>
            ))}
          </div>
      ...
      {/* 하단 액션 버튼 (3번 박스 하단) - hideButtons가 false일 때만 표시 */}
      {!hideButtons && (
